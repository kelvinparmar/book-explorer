...
No API key is required to get started — the Google Books `volumes` endpoint
works unauthenticated. That said, unauthenticated requests share a low
quota across everyone on your network/IP, so you may see
`429 Too Many Requests` under moderate use (common on shared dev networks,
VPNs, or CI). The app already retries 429s and 5xxs with backoff, but if
you're hitting it often:

```bash
cp .env.example .env
# then paste a free key from https://console.cloud.google.com/apis/credentials
# (enable "Books API" on the project first) into VITE_GOOGLE_BOOKS_API_KEY
```
...
slow, stale response can't clobber a newer one. `429` (rate limited) and
`5xx` responses are retried up to twice with exponential backoff
(honoring a `Retry-After` header when present) before surfacing a
plain-language error; other statuses (e.g. a bad request) fail immediately
